import { useEffect, useState } from "react";
import axios from "axios";
import { Link } from "react-router-dom";
import "bootstrap/dist/css/bootstrap.min.css";
import { BiPlus, BiSolidPencil, BiSolidTrash } from "react-icons/bi";
import { Button, Modal } from "react-bootstrap";
// import { ToastContainer , Toast } from 'react-to'

function Achat() {
  const [achat, setAchat] = useState([]);
  const [deleteAchat, setDeleteAchat] = useState("");
  const [showModal, setshowModal] = useState(false);
  const [searchAchat, setsearchAchat] = useState("");
  const [avaible,setavaible] = useState("")

  useEffect(() => {
    fetch("http://127.0.0.1:5000/api/achat", {
      methods: "GET",
      headers: {
        "content-type": "applications/json",
      },
    })
      .then((resp) => resp.json())
      .then((resp) => console.log(resp))
      .catch((error) => console.log(error));
  });

  const [data, setData] = useState([]);
  // const {totalty} = useGlobalContext();
  const fetchAchat = () => {
    axios
      .get("http://127.0.0.1:5000/api/achat")
      .then((res) => setData(res.data))
      .catch((err) => console.log(err));
  };
  useEffect(() => {
    fetchAchat();
  }, []);
  const totalty = () => {
    let totalty = 0;
    data.forEach((datas) => {
      totalty = totalty + datas.pret;
    });
    return totalty;
  };
  const handleDelete = (id) => {
    setshowModal(true);
    setDeleteAchat(id);
    
  };
  // console.log('total' , totalty())
  // {data.map((item) => {
  //     const {pret} = item
  // })}*

  const supprimerAchat = () => {
    axios
      .delete("http://127.0.0.1:5000/api/achat/" + deleteAchat)
      .then((res) => {
        console.log(res.data);
        setshowModal(false);
        fetchAchat();
      })
      .catch((err) => console.log(err));
  };

  const filterData = data.filter((item) =>
    item.nomClient.toLowerCase().includes(searchAchat.toLocaleLowerCase())
  );
  const getValue = (e)=>{
    // setavaible(e.target.value)
    let tab = []
    if(tab.length>0){
    for(let i=0;i<tab.length;i++){
      console.log(tab[i]);

    }
    }
    
    
  }


  return (
    <div className="w-100 bg-red rounded p-3 top-6">
       <h2 className="text-center">LISTES DES ACHATS</h2>
      <Modal show={showModal} centered>
        <Modal.Header>
          <Modal.Title>VOUS VOULEZ SUPPRIMER</Modal.Title>
        </Modal.Header>
        <Modal.Footer>
          <Button onClick={() => setshowModal(false)}>NON</Button>
          <Button variant="danger" onClick={supprimerAchat}>
            OUI
          </Button>
        </Modal.Footer>
      </Modal>
      <div
        className=""
        style={{ display: "flex", justifyContent: "space-between" }}
      >
        <Link to="/achat/ajouter" className="btn btn-lg btn-success ">
          + Ajouter
        </Link>
        <input
          type="text"
          placeholder="Recherche..."
          style={{ width: "350px", outline: "none" }}
          value={searchAchat}
          onChange={(e)=>setsearchAchat(e.target.value)}
        />
      </div>
      <table className="table overflow-x:auto table-borderless ">
        <thead>
          <tr>
            {/* { <th>numCompte</th> }
            { <th scope="col" class="px-6 py-3">
              ID
            </th> } */}
            <th></th>
            <th scope="col" class="px-6 py-3">
              NOM DU MEDICAMENT
            </th>
            <th scope="col" class="px-6 py-3">
              NOM DU CLIENT
            </th>
            <th scope="col" class="px-6 py-3">
              QUANTITES
            </th>
            <th scope="col" class="px-6 py-3">
              DATE
            </th>
            {/* <th scope="col" class="px-6 py-3">DATE</th>
                        <th scope="col" class="px-6 py-3">MONTANT à PAYER</th> */}
            <th scope="col" colspan="2" class="px-6 py-3">
              ACTION
            </th>
          </tr>
        </thead>
        <tbody>
          {filterData.map((achat, index) => {
            const { pret } = achat;
            return (
              <tr key={index}>
                <td>
                <input type="checkbox" value={index} onChange={getValue}/>
                </td>
                <td>{achat.med}</td>
                {/*    */}
                <td scope="col" class="px-6 py-3">
                  {achat.nomClient}
                </td>
                <td scope="col" class="px-6 py-3">
                  {achat.nombre}
                </td>
                <td scope="col" class="px-6 py-3">
                  {achat.date}
                </td>
                {/* <td scope="col" class="px-6 py-3">{achat.date}</td>
                                <td scope="col" class="px-6 py-3">{achat.pret}</td> */}
                {/* <td scope="col" class="px-6 py-3">{pret}</td> */}
                <td scope="col" class="px-2 py-1flex">
                  {/* <Link  to={`/read/${achat.id}`} className='btn btn-sm btn-primary'>read</Link> */}
                  <Link
                    to={`/achat/edit/${achat.idA}`}
                    className="btn btn-lg btn-success"
                  >
                    <BiSolidPencil />
                  </Link>
                  <button
                    className="btn btn-lg btn-danger"
                    onClick={() => handleDelete(achat.idA)}
                    style={{ marginLeft: "10px" }}
                  >
                    <BiSolidTrash />
                  </button>
                </td>
              </tr>
            );
          })}
        </tbody>
      </table>
      <Link to="facture" className="btn btn-lg btn-success ">
          PDF
        </Link>
    </div>
  );
}
export default Achat;
